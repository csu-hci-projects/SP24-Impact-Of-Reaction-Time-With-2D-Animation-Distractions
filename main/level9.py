...
def init():
    global circles
    top_to_bottom_positions = [(5, 17), (15, 19)]
    left_to_right_position = [(0, 10)]
    right_to_left_position = [(10, 15)]
    bottom_to_top_positions = [(10, 0), (16, 0)]
    
    circles = [createCircle(ax, *pos) for pos in top_to_bottom_positions + left_to_right_position + right_to_left_position + bottom_to_top_positions]
    return circles
...
def objectFound(click):
    ...
    if abs(click.xdata - searchItem_x) < tolerance and abs(click.ydata - searchItem_y) < tolerance:
        ...
        with open('level9.txt', 'w') as file:
